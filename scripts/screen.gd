...
onready var base_size = Vector2(1024, 768)
var settings

func _ready():
	settings = get_node("/root/PlayerVariables")
	base_size = settings.resolutions[settings.settings["res"]]
...
#		# I set the windowed version to an arbitrary 80% of screen size here
	...
	var window_x = (window_size.x / 2) - (base_size.x / 2)
	var window_y = (window_size.y / 2) - (base_size.y / 2)
	OS.set_borderless_window(false)
	OS.set_window_fullscreen(false)
	OS.set_window_position(Vector2(window_x, window_y))
	OS.set_window_size(base_size)
#func set_windowed():
#	var window_size = OS.get_screen_size()
#		# I set the windowed version to an arbitrary 80% of screen size here
